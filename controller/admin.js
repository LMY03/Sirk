...
const admin = {
    ...
    createIsyu: async function(req, res) {
        ...
        const isyuFolderName = "0" + (model.issues.length + 1) + ". " + isyuName + " ISYU";
        ...
        for (const [index, section] of sections.entries()) {
            ...
            for (const folder of model.oSirkFolders) section.oFolders.push(await google.createFolder(folder.name, section.oID));
            // section.mID = await google.createFolder(`0${index + 1}. ${section.name}`, issue.mSirk.folderID);
            // for (const folder of model.mSirkFolders) section.mFolders.push(await google.createFolder(folder.name, section.mID));
        }
